			...
			?>
				<div class="notice-content">
					<?php if ( ! storefront_is_woocommerce_activated() && current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) : ?>
						<h2><?php esc_html_e( 'Thanks for installing Storefront, you rock! 🤘', 'storefront lumione' ); ?></h2>
						<p><?php esc_html_e( 'To enable eCommerce features you need to install the WooCommerce plugin.', 'storefront lumione' ); ?></p>
						<p><?php Storefront_Plugin_Install::install_plugin_button( 'woocommerce', 'woocommerce.php', 'WooCommerce', array(), __( 'WooCommerce activated', 'storefront lumione' ), __( 'Activate WooCommerce', 'storefront lumione' ), __( 'Install WooCommerce', 'storefront lumione' ) ); ?></p>
					<?php endif; ?>

					<?php if ( storefront_is_woocommerce_activated() ) : ?>
						<h2><?php esc_html_e( 'Design your store 🎨', 'storefront lumione' ); ?></h2>
						<p>
							<?php
							if ( true === (bool) get_option( 'storefront_nux_fresh_site' ) && 'post-new.php' === $pagenow ) {
								echo esc_html__( 'Before you add your first product let\'s design your store. We\'ll add some example products for you. When you\'re ready let\'s get started by adding your logo.', 'storefront lumione' );
							} else {
								echo esc_html__( 'You\'ve set up WooCommerce, now it\'s time to give it some style! Let\'s get started by entering the Customizer and adding your logo.', 'storefront lumione' );
							}
							?>
						</p>
								...
								<div class="notice-input">
									<label>
										...
										if ( 'page' === get_option( 'show_on_front' ) ) {
											esc_html_e( 'Apply the Storefront homepage template', 'storefront lumione' );
										} else {
											esc_html_e( 'Create a homepage using Storefront\'s homepage template', 'storefront lumione' );
										}
										?>
									</label>
								</div>
								...
								<?php if ( self::is_woocommerce_empty() ) : ?>
									<div class="notice-input">
										<label>
											...
											<?php esc_html_e( 'Add example products', 'storefront lumione' ); ?>
										</label>
									</div>
								<?php endif; ?>
							<?php endif; ?>

							<input type="submit" name="storefront-guided-tour" class="sf-nux-button" value="<?php esc_attr_e( 'Let\'s go!', 'storefront lumione' ); ?>">
							<a href="#" class="sf-nux-dismiss-button" ><?php esc_html_e( 'Skip', 'storefront lumione' ); ?></a>
						</form>
					<?php endif; ?>
				</div>
			<?php
