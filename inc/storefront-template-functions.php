	...
	function storefront_comment( $comment, $args, $depth ) {
		...
		<div class="comment-meta commentmetadata">
			...
			<?php if ( '0' === $comment->comment_approved ) : ?>
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'storefront lumione' ); ?></em>
				<br />
			<?php endif; ?>
		...
		?>
		<?php edit_comment_link( __( 'Edit', 'storefront lumione' ), '  ', '' ); ?>
		</div>
		</div>
		<?php if ( 'div' !== $args['style'] ) : ?>
		</div>
	...
	function storefront_credit() {
		...
		if ( apply_filters( 'storefront_credit_link', true ) ) {
			if ( storefront_is_woocommerce_activated() ) {
				$links_output .= '<a href="https://woocommerce.com" target="_blank" title="' . esc_attr__( 'WooCommerce - The Best eCommerce Platform for WordPress', 'storefront lumione' ) . '" rel="noreferrer nofollow">' . esc_html__( 'Built with WooCommerce', 'storefront lumione' ) . '</a>.';
			} else {
				$links_output .= '<a href="https://woocommerce.com/products/storefront/" target="_blank" title="' . esc_attr__( 'Storefront -  The perfect platform for your next WooCommerce project.', 'storefront lumione' ) . '" rel="noreferrer nofollow">' . esc_html__( 'Built with Storefront', 'storefront lumione' ) . '</a>.';
			}
		}
...
if ( ! function_exists( 'storefront_primary_navigation' ) ) {
	...
	function storefront_primary_navigation() {
		?>
		<nav id="site-navigation" class="main-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Primary Navigation', 'storefront lumione' ); ?>">
		<button id="site-navigation-menu-toggle" class="menu-toggle" aria-controls="site-navigation" aria-expanded="false"><span><?php echo esc_html( apply_filters( 'storefront_menu_toggle_text', __( 'Menu', 'storefront lumione' ) ) ); ?></span></button>
			<?php
			wp_nav_menu(
				array(
...
if ( ! function_exists( 'storefront_secondary_navigation' ) ) {
	...
	function storefront_secondary_navigation() {
		if ( has_nav_menu( 'secondary' ) ) {
			?>
			<nav class="secondary-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Secondary Navigation', 'storefront lumione' ); ?>">
				<?php
					wp_nav_menu(
						array(
...
if ( ! function_exists( 'storefront_skip_links' ) ) {
	...
	function storefront_skip_links() {
		?>
		<a class="skip-link screen-reader-text" href="#site-navigation"><?php esc_html_e( 'Skip to navigation', 'storefront lumione' ); ?></a>
		<a class="skip-link screen-reader-text" href="#content"><?php esc_html_e( 'Skip to content', 'storefront lumione' ); ?></a>
		<?php
	}
}
...
if ( ! function_exists( 'storefront_homepage_header' ) ) {
	...
	function storefront_homepage_header() {
		edit_post_link( __( 'Edit this section', 'storefront lumione' ), '', '', '', 'button storefront-hero__button-edit' );
		?>
	...
	function storefront_page_content() {
		...
		<div class="entry-content">
			...
			<?php
				wp_link_pages(
					array(
						'before' => '<div class="page-links">' . __( 'Pages:', 'storefront lumione' ),
						'after'  => '</div>',
					)
				);
			?>
	...
	function storefront_post_content() {
		...
		the_content(
			sprintf(
				...
				__( 'Continue reading %s', 'storefront lumione' ),
...
if ( ! function_exists( 'storefront_post_meta' ) ) {
	...
	function storefront_post_meta() {
		...
		$posted_on = '
			...
			sprintf( __( 'Posted on %s', 'storefront lumione' ), $output_time_string ) .
			'</span>';

		// Author.
		$author = sprintf(
			'<span class="post-author">%1$s <a href="%2$s" class="url fn" rel="author">%3$s</a></span>',
			__( 'by', 'storefront lumione' ),
		...
		if ( ! post_password_required() && ( comments_open() || 0 !== intval( get_comments_number() ) ) ) {
			$comments_number = get_comments_number_text( __( 'Leave a comment', 'storefront lumione' ), __( '1 Comment', 'storefront lumione' ), __( '% Comments', 'storefront lumione' ) );
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'storefront lumione' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'storefront_post_taxonomy' ) ) {
	...
	function storefront_post_taxonomy() {
		...
		$categories_list = get_the_category_list( __( ', ', 'storefront lumione' ) );

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'storefront lumione' ) );
		?>

		<aside class="entry-taxonomy">
			<?php if ( $categories_list ) : ?>
			<div class="cat-links">
				<?php echo esc_html( _n( 'Category:', 'Categories:', count( get_the_category() ), 'storefront lumione' ) ); ?> <?php echo wp_kses_post( $categories_list ); ?>
			</div>
			<?php endif; ?>

			<?php if ( $tags_list ) : ?>
			<div class="tags-links">
				<?php echo esc_html( _n( 'Tag:', 'Tags:', count( get_the_tags() ), 'storefront lumione' ) ); ?> <?php echo wp_kses_post( $tags_list ); ?>
			</div>
			<?php endif; ?>
		</aside>
...
if ( ! function_exists( 'storefront_paging_nav' ) ) {
	...
	function storefront_paging_nav() {
		...
		$args = array(
			'type'      => 'list',
			'next_text' => _x( 'Next', 'Next post', 'storefront lumione' ),
			'prev_text' => _x( 'Previous', 'Previous post', 'storefront lumione' ),
		);
...
if ( ! function_exists( 'storefront_post_nav' ) ) {
	...
	function storefront_post_nav() {
		$args = array(
			'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next post:', 'storefront lumione' ) . ' </span>%title',
			'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous post:', 'storefront lumione' ) . ' </span>%title',
		);
