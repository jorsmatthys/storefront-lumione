...
if ( ! function_exists( 'storefront_cart_link' ) ) {
	...
	function storefront_cart_link() {
		...
		?>
			<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'storefront lumione' ); ?>">
				<?php /* translators: %d: number of items in cart */ ?>
				<?php echo wp_kses_post( WC()->cart->get_cart_subtotal() ); ?> <span class="count"><?php echo wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'storefront lumione' ), WC()->cart->get_cart_contents_count() ) ); ?></span>
			</a>
		<?php
	}
}
	...
	function storefront_product_categories( $args ) {
		$args = apply_filters(
			...
			array(
				...
				'child_categories' => 0,
				'orderby'          => 'menu_order',
				'title'            => __( 'Shop by Category', 'storefront lumione' ),
			)
		);
		...
		if ( false !== strpos( $shortcode_content, 'product-category' ) ) {
			echo '<section class="storefront-product-section storefront-product-categories" aria-label="' . esc_attr__( 'Product Categories', 'storefront lumione' ) . '">';
	...
	function storefront_recent_products( $args ) {
		$args = apply_filters(
			...
			array(
				...
				'orderby' => 'date',
				'order'   => 'desc',
				'title'   => __( 'New In', 'storefront lumione' ),
			)
		);
		...
		if ( false !== strpos( $shortcode_content, 'product' ) ) {
			echo '<section class="storefront-product-section storefront-recent-products" aria-label="' . esc_attr__( 'Recent Products', 'storefront lumione' ) . '">';
	...
	function storefront_featured_products( $args ) {
		$args = apply_filters(
			...
			array(
				...
				'order'      => 'desc',
				'visibility' => 'featured',
				'title'      => __( 'We Recommend', 'storefront lumione' ),
			)
		);
		 ...
		 */
		if ( false !== strpos( $shortcode_content, 'product' ) ) {
			echo '<section class="storefront-product-section storefront-featured-products" aria-label="' . esc_attr__( 'Featured Products', 'storefront lumione' ) . '">';
	...
	function storefront_popular_products( $args ) {
		$args = apply_filters(
			...
			array(
				...
				'orderby' => 'rating',
				'order'   => 'desc',
				'title'   => __( 'Fan Favorites', 'storefront lumione' ),
			)
		);
		 ...
		 */
		if ( false !== strpos( $shortcode_content, 'product' ) ) {
			echo '<section class="storefront-product-section storefront-popular-products" aria-label="' . esc_attr__( 'Popular Products', 'storefront lumione' ) . '">';
	...
	function storefront_on_sale_products( $args ) {
		$args = apply_filters(
			...
			array(
				...
				'order'   => 'desc',
				'on_sale' => 'true',
				'title'   => __( 'On Sale', 'storefront lumione' ),
			)
		);
		 ...
		 */
		if ( false !== strpos( $shortcode_content, 'product' ) ) {
			echo '<section class="storefront-product-section storefront-on-sale-products" aria-label="' . esc_attr__( 'On Sale Products', 'storefront lumione' ) . '">';
	...
	function storefront_best_selling_products( $args ) {
		$args = apply_filters(
			...
			array(
				...
				'orderby' => 'popularity',
				'order'   => 'desc',
				'title'   => esc_attr__( 'Best Sellers', 'storefront lumione' ),
			)
		);
		 ...
		 */
		if ( false !== strpos( $shortcode_content, 'product' ) ) {
			echo '<section class="storefront-product-section storefront-best-selling-products" aria-label="' . esc_attr__( 'Best Selling Products', 'storefront lumione' ) . '">';
	...
	function storefront_promoted_products( $per_page = '2', $columns = '2', $recent_fallback = true ) {
		if ( storefront_is_woocommerce_activated() ) {
			...
			if ( wc_get_featured_product_ids() ) {
				...
				echo '<h2>' . esc_html__( 'Featured Products', 'storefront lumione' ) . '</h2>';
				...
				echo '<h2>' . esc_html__( 'On Sale Now', 'storefront lumione' ) . '</h2>';
				...
				echo '<h2>' . esc_html__( 'New In Store', 'storefront lumione' ) . '</h2>';
...
if ( ! function_exists( 'storefront_handheld_footer_bar_search' ) ) {
	...
	function storefront_handheld_footer_bar_search() {
		echo '<a href="">' . esc_attr__( 'Search', 'storefront lumione' ) . '</a>';
...
if ( ! function_exists( 'storefront_handheld_footer_bar_cart_link' ) ) {
	...
	function storefront_handheld_footer_bar_cart_link() {
		...
		?>
			<a class="footer-cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>"><?php esc_html_e( 'Cart', 'storefront lumione' ); ?>
...
if ( ! function_exists( 'storefront_handheld_footer_bar_account_link' ) ) {
	...
	function storefront_handheld_footer_bar_account_link() {
		echo '<a href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'My Account', 'storefront lumione' ) . '</a>';
	}
}
...
if ( ! function_exists( 'storefront_single_product_pagination' ) ) {
	...
	function storefront_single_product_pagination() {
		...
		?>
		<nav class="storefront-product-pagination" aria-label="<?php esc_attr_e( 'More products', 'storefront lumione' ); ?>">
				...
				<div class="col-full">
					<div class="storefront-sticky-add-to-cart__content">
						...
						<div class="storefront-sticky-add-to-cart__content-product-info">
							<span class="storefront-sticky-add-to-cart__content-title"><?php esc_html_e( 'You\'re viewing:', 'storefront lumione' ); ?> <strong><?php the_title(); ?></strong></span>
	...
	function storefront_woocommerce_brands_homepage_section() {
		$args = apply_filters(
			...
			array(
				...
				'orderby'    => 'name',
				'show_empty' => false,
				'title'      => __( 'Shop by Brand', 'storefront lumione' ),
			)
		);
		...
		echo '<section class="storefront-product-section storefront-woocommerce-brands" aria-label="' . esc_attr__( 'Product Brands', 'storefront lumione' ) . '">';
